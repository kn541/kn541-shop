/**
 * KN541 쇼핑몰 네비게이션 데이터
 * - 카테고리 드롭다운: API에서 동적 로드 (실패 시 더미 폴백)
 * - 메인 네비게이션: KN541 쇼핑몰 메뉴 구조로 업데이트
 */
...
const DUMMY_CATEGORIES = [
  ...
  { name: 'T-Shirts', handle: 'page-style-2/t-shirts', description: 'Perfect for gentlemen', icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="size-8" width="24" height="24" color="currentColor" fill="none"><path d="M6 9V16.6841C6 18.4952 6 19.4008 6.58579 19.9635C7.89989 21.2257 15.8558 21.4604 17.4142 19.9635C18 19.4008 18 18.4952 18 16.6841V9" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"></path></svg>` },
...
export async function getNavigation(): Promise<TNavigationItem[]> {
  return [
    ...
    { id: '4', href: '/collections/all', name: 'Sale' },
    {
      ...
      children: [
        {
          id: '1', href: '/', name: 'Home Page', type: 'dropdown',
          children: [
            { id: '1-1', href: '/', name: 'Home 1' },
            { id: '1-2', href: '/home-2', name: 'Home 2' },
        ...
        {
          id: '2', href: '/#', name: 'Shop Pages', type: 'dropdown',
          children: [
        ...
        {
          id: '3', href: '/#', name: 'Other Pages', type: 'dropdown',
          children: [
        ...
        {
          id: '4', href: '/#', name: 'Other Pages', type: 'dropdown',
          children: [
    ...
    },
    {
      ...
      children: [
        ...
        { id: 'gid://6', href: '/orders', name: 'Orders' },
        { id: '7', href: '/search', name: 'Search Page' },
        { id: '8', href: '/account', name: 'Account Page' },
        { id: '9', href: '/about', name: 'Other Pages', type: 'dropdown', children: [{ id: '9-1', href: '/about', name: 'About' }, { id: '9-2', href: '/contact', name: 'Contact us' }, { id: '9-3', href: '/login', name: 'Login' }, { id: '9-4', href: '/signup', name: 'Signup' }, { id: '9-5', href: '/subscription', name: 'Subscription' }, { id: '9-6', href: '/forgot-pass', name: 'Forgot Password' }] },
...
/**
 * 헤더 카테고리 드롭다운
 * - API에서 카테고리 목록을 가져와 드롭다운 형식으로 변환
 * - API 실패 시 더미 카테고리 폴백
 */
export const getHeaderDropdownCategories = async () => {
  ...
  if (apiResult?.items?.length) {
    // API 카테고리를 드롭다운 형식으로 변환
    return apiResult.items.slice(0, 8).map((cat) => ({
      name: cat.name,
      handle: cat.handle,
      description: cat.description || `${cat.product_count ?? 0} products`,
      // 아이콘은 더미 카테고리에서 이름 매칭 시 재사용, 없으면 기본 아이콘
