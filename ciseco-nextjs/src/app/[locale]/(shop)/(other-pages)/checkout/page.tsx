...
// fix: 배송메모 직접입력 커서 사라짐 — MemoInput 컴포넌트 외부 분리 (#업무-온라인오픈 6번)
//   원인: MemoInput이 return 이후에 선언 → 매 렌더마다 새 컴포넌트 생성 → input unmount/remount → 커서 유실
//   수정: MemoInput을 CheckoutPage 외부 named 컴포넌트로 분리 → React가 동일 컴포넌트로 인식해 remount 없음
...
// 내부 const MemoInput = () => ... 는 매 렌더마다 새 컴포넌트 타입 생성 → unmount/remount → 커서 유실
// 외부 정의 시 컴포넌트 참조 안정 → re-render만 발생, 커서 유지됨
interface MemoInputProps {
  inputCls: string
  labelCls: string
  memoSelect: string
  memo: string
  onMemoSelectChange: (val: string) => void
  onMemoChange: (val: string) => void
}

function MemoInput({ inputCls, labelCls, memoSelect, memo, onMemoSelectChange, onMemoChange }: MemoInputProps) {
  return (
    <div className="sm:col-span-2">
      <label className={labelCls}>배송 메모 (선택)</label>
      <select className={inputCls} value={memoSelect} onChange={e => onMemoSelectChange(e.target.value)}>
        {MEMO_OPTIONS.map(o => (
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    if (!token) router.replace(`/${locale}`)
  }, [locale, router])

  const orderableItems = items.filter(i => selectedIds.has(i.id) && i.productId?.includes('-'))
  const skippedCount = items.filter(i => !i.productId?.includes('-')).length

  const isDigitalOnly = orderableItems.length > 0 && orderableItems.every(i => i.product_type === '005')
  ...
  const summaryTotal  = isDigitalOnly ? orderTotal : orderTotal + orderShipping
  ...
  }, [form.name, form.phone, address.address1, address.address2, address.zipcode, orderableItems.length])

  useEffect(() => {
    if (!mounted || !isDigitalOnly) return
    const token = getToken()
    if (!token) return
    let cancelled = false
    ...
    ;(async () => {
      try {
        ...
        const info = { name: String(d?.name ?? d?.full_name ?? ''), phone: String(d?.phone ?? ''), email: String(d?.email ?? '') }
        if (cancelled) return
        setMemberInfo(info)
        setForm(f => ({ ...f, name: info.name, phone: info.phone, email: info.email || f.email }))
      } catch {
        if (!cancelled) toast.error('회원 정보를 불러오지 못했습니다.')
      } finally {
  ...
  useEffect(() => {
    if (!mounted || !getToken() || paymentCompleteRef.current) return
  ...
  useEffect(() => {
    ...
    async function initPayment() {
      try {
        const token = getToken()
        const configRes = await fetch(apiUrl('/payments/config'), { headers: token ? { Authorization: `Bearer ${token}` } : {} })
        ...
        if (token) {
          try {
            const meRes = await fetch(apiUrl('/auth/me'), { headers: { Authorization: `Bearer ${token}` } })
            if (meRes.ok) { const uid = (await meRes.json()).data?.user_id; if (uid) customerKey = uid }
          } catch {}
        }
  ...
  const handlePay = async () => {
    ...
    const payName = form.name.trim()
    const payPhone = form.phone.trim()
    const payEmail = form.email.trim()
    ...
    try {
      ...
      if (!isDigitalOnly && showNewForm && saveNewAddress) {
        try {
          await fetch(apiUrl('/my/addresses'), { method: 'POST', headers, body: JSON.stringify({ recipient_name: form.name.trim(), recipient_phone: form.phone.trim(), zip_code: address.zipcode, address1: address.address1, address2: address.address2 ?? '', delivery_memo: form.memo || null, is_default: savedAddresses.length === 0 }) })
        } catch {}
      }
      ...
      const orderName = orderableItems.length === 1 ? orderableItems[0].name : `${orderableItems[0].name} 외 ${orderableItems.length - 1}건`
      const origin = window.location.origin

      // ── 토스페이 (간편결제) ─────────────────────────────────────────────
      if (payMethod === 'EASY_PAY') {
        pendingOrderRef.current = null
        const tosspayRes = await fetch(apiUrl('/payments/tosspay/create'), { method: 'POST', headers, body: JSON.stringify({ order_id, amount: Math.round(total_amount), order_name: orderName, ret_url: `${origin}/${locale}/payment/tosspay/success?internal_order_id=${order_id}`, ret_cancel_url: `${origin}/${locale}/payment/tosspay/cancel?internal_order_id=${order_id}` }) })
      ...
      // ── 토스페이먼츠 (카드 등) ──────────────────────────────────────────
      const prepareRes  = await fetch(apiUrl('/payments/prepare'), { method: 'POST', headers, body: JSON.stringify({ order_id, amount: Math.round(total_amount), order_name: orderName }) })
      const prepareData = await prepareRes.json()
      if (!prepareRes.ok) throw new Error(prepareData.detail ?? '결제 사전등록에 실패했습니다')

      const baseParams = { orderId: order_no, orderName, customerName: payName, customerEmail: payEmail || undefined, customerMobilePhone: payPhone.replace(/[^0-9]/g, ''), successUrl: `${origin}/${locale}/payment/success?internal_order_id=${order_id}`, failUrl: `${origin}/${locale}/payment/fail?internal_order_id=${order_id}`, amount: { currency: 'KRW', value: Math.round(total_amount) } as const }

      if (payMethod === 'CARD') {
        await paymentRef.current.requestPayment({ method: 'CARD', ...baseParams })
      } else if (payMethod === 'VIRTUAL_ACCOUNT') {
        await paymentRef.current.requestPayment({ method: 'VIRTUAL_ACCOUNT', ...baseParams, virtualAccount: { cashReceipt: { type: '미발행' }, useEscrow: false } })
      } else {
      ...
      if (!isCancellation) { pendingOrderRef.current = null; toast.error(msg) }
  ...
  // MemoInput에 전달할 props 공통 객체
  const memoProps: MemoInputProps = {
    inputCls, labelCls, memoSelect, memo: form.memo,
    onMemoSelectChange: handleMemoSelect,
    onMemoChange: (val) => setForm(f => ({ ...f, memo: val })),
  }
        ...
        <div className="flex-1 space-y-8">
          {isDigitalOnly && (
            <section className="rounded-3xl border border-neutral-200 p-6 dark:border-neutral-700">
              ...
              {digitalOrdererLoading ? (
                <div className="flex justify-center py-10"><div className="h-8 w-8 animate-spin rounded-full border-2 border-primary-500 border-t-transparent" /></div>
              ) : (
                <>
                  <p className="mb-4 rounded-xl border border-neutral-200 bg-neutral-50 px-4 py-3 text-sm text-neutral-600 dark:border-neutral-600 dark:bg-neutral-800/50 dark:text-neutral-300">디지털 콘텐츠 상품으로 배송이 필요하지 않습니다.</p>
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div className="sm:col-span-2">
                      <label className={labelCls}>이름</label>
                      <input className={`${inputCls} cursor-not-allowed bg-neutral-100 opacity-90 dark:bg-neutral-800`} type="text" value={form.name} readOnly disabled autoComplete="name" />
                    </div>
                    <div className="sm:col-span-2">
                      <label className={labelCls}>휴대폰</label>
                      <input className={`${inputCls} cursor-not-allowed bg-neutral-100 opacity-90 dark:bg-neutral-800`} type="tel" value={form.phone} readOnly disabled autoComplete="tel" />
                    </div>
                    <div className="sm:col-span-2">
                      <label className={labelCls}>이메일 (영수증·주문 확인 발송)</label>
                      <input className={inputCls} placeholder="example@email.com" type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} autoComplete="email" />
                    </div>
                  </div>
                </>
              )}
          ...
          <section className="rounded-3xl border border-neutral-200 p-6 dark:border-neutral-700">
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              <button type="button" onClick={() => { setShowNewForm(true); setSameAsMember(false); setSelectedAddressId(null); setMemoSelect(''); setForm(f => ({ ...f, name: '', phone: '', memo: '' })); setAddress({ zipcode: '', address1: '', address2: '' }) }} className={`rounded-xl px-4 py-2 text-sm font-medium transition-all ${showNewForm ? 'bg-primary-600 text-white' : 'border border-neutral-200 text-neutral-600 hover:border-neutral-300 dark:border-neutral-600 dark:text-neutral-300'}`}>새 배송지 입력</button>
              <button type="button" onClick={() => { setShowNewForm(false); setSameAsMember(false); if (savedAddresses.length > 0) { const sel = savedAddresses.find(a => a.id === selectedAddressId) ?? savedAddresses[0]; setSelectedAddressId(sel.id); applyAddress(sel) } }} className={`rounded-xl px-4 py-2 text-sm font-medium transition-all ${!showNewForm ? 'bg-primary-600 text-white' : 'border border-neutral-200 text-neutral-600 hover:border-neutral-300 dark:border-neutral-600 dark:text-neutral-300'}`}>저장된 배송지 ({savedAddresses.length})</button>
            </div>
            ...
            {!showNewForm && savedAddresses.length > 0 && (
              <div className="mb-4 space-y-2">
                {savedAddresses.map(addr => (
                  <button key={addr.id} type="button" onClick={() => handleSelectAddress(addr)} className={`w-full rounded-2xl border p-4 text-left transition-all ${selectedAddressId === addr.id ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20' : 'border-neutral-200 hover:border-neutral-300 dark:border-neutral-700'}`}>
            ...
            {showNewForm && (
              <div className="space-y-4 pt-2">
                <label className="flex cursor-pointer items-center gap-2 rounded-xl border border-primary-200 bg-primary-50 px-4 py-3 text-sm font-medium text-primary-700 dark:border-primary-800 dark:bg-primary-900/20 dark:text-primary-300">
                  <input type="checkbox" checked={sameAsMember} onChange={async e => {
                    const checked = e.target.checked; setSameAsMember(checked)
                    if (checked) {
                      if (memberInfo) { setForm(f => ({ ...f, name: memberInfo.name, phone: memberInfo.phone, email: memberInfo.email })) }
                      else { try { const token = getToken(); if (!token) return; const res = await fetch(apiUrl('/auth/me'), { headers: { Authorization: `Bearer ${token}` } }); if (res.ok) { const d = (await res.json()).data; const info = { name: d?.name ?? '', phone: d?.phone ?? '', email: d?.email ?? '' }; setMemberInfo(info); setForm(f => ({ ...f, name: info.name, phone: info.phone, email: info.email })) } } catch {} }
                    } else { setForm(f => ({ ...f, name: '', phone: '', email: '' })) }
                  }} className="h-4 w-4 rounded border-primary-300 text-primary-600" />
                  회원정보와 동일
                </label>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div className="sm:col-span-2"><label className={labelCls}>수령자 이름 *</label><input className={inputCls} placeholder="홍길동" type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
                  <div><label className={labelCls}>휴대폰 *</label><input className={inputCls} placeholder="010-0000-0000" type="tel" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} /></div>
                  <div><label className={labelCls}>이메일 (선택)</label><input className={inputCls} placeholder="example@email.com" type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} /></div>
                  <div className="sm:col-span-2"><KakaoAddressInput value={address} onChange={setAddress} label="주소 *" inputClassName={inputCls} labelClassName={labelCls} /></div>
                  <MemoInput {...memoProps} />
                </div>
                {getToken() && (
                  <label className="flex cursor-pointer items-center gap-2 text-sm text-neutral-600 dark:text-neutral-400">
                    <input type="checkbox" checked={saveNewAddress} onChange={e => setSaveNewAddress(e.target.checked)} className="h-4 w-4 rounded border-neutral-300 text-primary-600" />
                    이 배송지를 저장하기
                  </label>
                )}
              </div>
            ...
            {!showNewForm && selectedAddressId && (
              <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div className="sm:col-span-2"><label className={labelCls}>이메일 (선택)</label><input className={inputCls} placeholder="example@email.com" type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} /></div>
                <MemoInput {...memoProps} />
              </div>
            )}
          </section>
          ...
          <section className="rounded-3xl border border-neutral-200 p-6 dark:border-neutral-700">
            ...
            <div className="grid grid-cols-3 gap-3">
              {PAY_METHODS.map(m => (
                <button key={m.key} onClick={() => setPayMethod(m.key)} className={`flex flex-col items-center gap-2 rounded-2xl border p-4 text-center transition-all ${payMethod === m.key ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20' : 'border-neutral-200 hover:border-neutral-300 dark:border-neutral-700'}`}>
            ...
            {payMethod === 'BANK_TRANSFER' && (
              <div className="mt-5 rounded-2xl border-2 border-amber-300 bg-amber-50 p-5 dark:border-amber-700 dark:bg-amber-900/20">
                <div className="mb-3 flex items-center gap-2"><BuildingLibraryIcon className="h-5 w-5 text-amber-600" /><p className="font-bold text-amber-800 dark:text-amber-300">무통장 입금 계좌 안내</p></div>
                <div className="space-y-2.5 text-sm">
                  <div className="flex flex-col gap-0.5 sm:flex-row sm:justify-between"><span className="text-amber-700 dark:text-amber-400">은행</span><span className="font-semibold text-amber-900 dark:text-amber-200">{BANK_ACCOUNT.bank}</span></div>
                  <div className="flex flex-col gap-0.5 sm:flex-row sm:justify-between"><span className="text-amber-700 dark:text-amber-400">계좌번호</span><span className="font-bold text-amber-900 dark:text-amber-200 break-all">{BANK_ACCOUNT.number}</span></div>
                  <div className="flex flex-col gap-0.5 sm:flex-row sm:justify-between"><span className="text-amber-700 dark:text-amber-400">예금주</span><span className="font-semibold text-amber-900 dark:text-amber-200">{BANK_ACCOUNT.holder}</span></div>
                  <div className="flex flex-col gap-0.5 sm:flex-row sm:justify-between border-t border-amber-200 pt-2.5 dark:border-amber-700"><span className="text-amber-700 dark:text-amber-400">입금금액</span><span className="text-xl font-bold text-amber-900 dark:text-amber-200">{summaryTotal.toLocaleString()}원</span></div>
                </div>
                <p className="mt-3 text-xs text-amber-600 dark:text-amber-500">⚠️ 주문 후 3일 이내 미입금 시 주문이 자동 취소됩니다.</p>
              </div>
            )}
          </section>
        </div>
        ...
        <div className="w-full lg:w-80 xl:w-96">
          <div className="sticky top-8 rounded-3xl border border-neutral-200 bg-neutral-50 p-6 dark:border-neutral-700 dark:bg-neutral-800">
              ...
              {orderableItems.map(item => (
                <div key={item.id} className="flex gap-3">
                  ...
                  <div className="flex flex-1 items-center justify-between">
                    ...
                    <p className="text-sm font-semibold">{((Number(item.price)||0)*(Number(item.quantity)||1)).toLocaleString()}원</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-2.5 text-sm text-neutral-600 dark:text-neutral-400">
              <div className="flex justify-between"><span>상품금액</span><span>{orderTotal.toLocaleString()}원</span></div>
              {!isDigitalOnly && (<div className="flex justify-between"><span>배송비</span><span className={orderShipping === 0 ? 'font-medium text-green-600' : ''}>{orderShipping === 0 ? '무료' : `${orderShipping.toLocaleString()}원`}</span></div>)}
            </div>
            ...
            <ButtonPrimary className="mt-6 w-full" onClick={handlePay} disabled={isSubmitting || (isDigitalOnly && digitalOrdererLoading)}>
              {isSubmitting ? (
                <span className="flex items-center gap-2"><svg className="h-4 w-4 animate-spin" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>처리 중...</span>
              ) : payMethod === 'BANK_TRANSFER' ? (
                <span className="flex items-center gap-2"><BuildingLibraryIcon className="h-4 w-4" />{summaryTotal.toLocaleString()}원 주문하기</span>
              ) : (
                <span className="flex items-center gap-2"><LockClosedIcon className="h-4 w-4" />{summaryTotal.toLocaleString()}원 결제하기</span>
              )}
            </ButtonPrimary>
            {payMethod === 'BANK_TRANSFER' && (<p className="mt-3 text-center text-xs text-amber-600">주문 후 {BANK_ACCOUNT.bank} {BANK_ACCOUNT.number}으로<br/>{summaryTotal.toLocaleString()}원 입금해 주세요</p>)}
            <div className="mt-4 flex items-center justify-center gap-1.5 text-xs text-neutral-400"><LockClosedIcon className="h-3.5 w-3.5" /><span>SSL 암호화 보호</span></div>
          </div>
        </div>
      </div>
    </main>
