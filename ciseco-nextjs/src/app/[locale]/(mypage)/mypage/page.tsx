...
export default function MypageHomePage() {
  ...
  return (
    <>
      ...
      <SectionHeader
        ...
        subtitle={
          shopPending
            ...
            : isL2 ? shop.shop_name : 가입 신청하면 나만의 쇼핑몰을 운영할 수 있어요'
        }
      />
      <div style={{ padding: '0 16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        {!isL2 && !shopPending && (
        ...
        {isL2 ? (
          ...
          <>
            ...
            <LockedCard
              ...
              benefitList={['템플릿 5개 선택', '로고 커스터마이징', '컨셋 색상 변경']}
              actionLabel='신청하기'
              actionHref={href('/mypage/shop/apply')}
            />
          </>
      ...
      <SectionHeader
        title='내 사업 (유료회원)'
        subtitle={isL3 ? `유효 기간: ${paid.expires_at ? new Date(paid.expires_at).toLocaleDateString('ko-KR') : '-'}까지` : '유료회원은 541 배당, 조직도, 출금 서비스를 이용할 수 있어요.'}
      />
      <div style={{ padding: '0 16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        {isL3 ? (
          // L3 활성 메뉴
          ...
          <>
            ...
            <LockedCard
              ...
              benefitList={['리스트븰 조직도', '트리뷰 조직도', '박스뷰 조직도']}
              actionLabel='유료회원 알아보기'
              actionHref={href('/mypage/upgrade-paid')}
            />
            <LockedCard
